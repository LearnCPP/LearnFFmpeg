...
//外部初始化，静态实例
const CWinLog* CWinLog::m_sInstance = NULL;
...
bool CWinLog::WriteLog(const LPCTSTR& szLog, const LPCTSTR& szFile,
	const int& line, const int& logtype/*=emErrorLog*/) const
{
	bool bRet = false;
	...
	{
	case emErrorLog:
		bRet = WriteLogInternal(szLog, szFile, line, m_strErrorLogName);
		break;
	case emRunLog:
