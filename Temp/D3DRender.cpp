...
bool CD3DRender::init_render(void* ctx, int w, int h, int pix_fmt)
{
	m_hwnd = (HWND)ctx;
	m_image_width = w;
	m_image_height = h;
...
bool CD3DRender::render_one_frame(AVFrame* data/*, AVFrame* info, int pix_fmt*/, const RECT& rect, bool bSel)
{
	HRESULT hr;
	...
	{
		destory_render();
		init_render(m_hwnd, data->width, data->height, 1);
	}

	const uint8_t* src_yuv[3] = { data->data[0], data->data[1], data->data[2] };
	...
	*/
	...
	if (bSel)
	{
		D3DXVECTOR2 LineArr[5];
		LineArr[0].x = (float)rect.left;
		LineArr[0].y = (float)rect.top;

		LineArr[1].x = (float)rect.right + 40;
		LineArr[1].y = (float)rect.top;

		LineArr[2].x = (float)rect.right + 40;
		LineArr[2].y = (float)rect.bottom;

		LineArr[3].x = (float)rect.left;
		LineArr[3].y = (float)rect.bottom;

		LineArr[4].x = (float)rect.left;
		LineArr[4].y = (float)rect.top;

		m_pLine->SetWidth(3.f);
		m_pLine->SetAntialias(TRUE);
		m_pLine->Draw(LineArr, 5, D3DCOLOR_RGBA(0, 255, 0, 255));
	}
	...
	hr = m_d3d_device->Present(NULL, &rect, NULL, NULL);
...
void CD3DRender::re_size(int width, int height)
{

}

void CD3DRender::aspect_ratio(int srcw, int srch, bool enable_aspect)
{
	if (enable_aspect)
	{
		enable_aspect = true;
...
void CD3DRender::destory_render()
{
...
void CD3DRender::fill_d3d_presentparams(D3DPRESENT_PARAMETERS *present_params)
{
...
bool CD3DRender::change_d3d_backbuffer(back_buffer_action_e action)
{
	HRESULT hr;
	...
	*/
	...
	if (m_d3d_handle)
	{
		...
		// 创建Direct3D线对象  
		if (FAILED(D3DXCreateLine(m_d3d_device, &m_pLine)))
		{
			destory_render();
			return false;
		}
...
bool CD3DRender::create_d3d_surfaces()
{
...
bool CD3DRender::reconfigure_d3d()
{
...
void CD3DRender::destroy_d3d_surfaces()
{
...
int CD3DRender::query_format(uint32_t movie_fmt)
{
	HRESULT hr;
	int i;
