...
void CMiniDumper::Enable(LPCTSTR pszAppName, bool bShowErrors)
{
	...
	_tcsncpy_s(m_szAppName, pszAppName, ARRSIZE(m_szAppName));
	...
	if (hDll)
	{
		if (pfnMiniDumpWriteDump)
		{
			...
			//COMMENTED by fengwen on 2006/11/15	<end> : 使用新的发送错误报告机制。
			{
				...
				_tcsncat_s(szBaseName, m_szAppName, ARRSIZE(szBaseName) - 1);
				...
				CString strFloderTime = time.Format(_T("LOG\\%Y-%m-%d"));
				CString strFileTime = time.Format(_T("\\%H_%M_%S_"));
				
				_tcsncat_s(szDumpPath, strFloderTime, strFloderTime.GetLength());
				AfxMessageBox(szDumpPath);
				if (!PathIsDirectory(szDumpPath))
				{//判断日期文件夹是否存在，不存在则创建
					SHCreateDirectoryEx(NULL, szDumpPath, 0);
				}
				_tcsncat_s(szDumpPath, strFileTime, strFileTime.GetLength());
				AfxMessageBox(szDumpPath);
				_tcsncat_s(szDumpPath, szBaseName, ARRSIZE(szDumpPath) - 1);
				_tcsncat_s(szDumpPath, _T(".dmp"), ARRSIZE(szDumpPath) - 1);
				AfxMessageBox(szDumpPath);
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					{
						// Do *NOT* localize that string (in fact, do not use MFC to load it)!
						_sntprintf_s(szResult, ARRSIZE(szResult), _T("Failed to save dump file to \"%s\".\r\n\r\nError: %u"), szDumpPath, GetLastError());
					}
					CloseHandle(hFile);
				}
				else
				{
					// Do *NOT* localize that string (in fact, do not use MFC to load it)!
					_sntprintf_s(szResult, ARRSIZE(szResult), _T("Failed to create dump file \"%s\".\r\n\r\nError: %u"), szDumpPath, GetLastError());
				}
			}
		}
		FreeLibrary(hDll);
