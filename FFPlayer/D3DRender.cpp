...
bool CD3DRender::render_one_frame(FFVideoData* data, FFVideoInfo* info, int pix_fmt, const CRect& rect, bool bSel)
{
	HRESULT hr;

	const uint8_t* src_yuv[3] = { data->pY, data->pU, data->pV };


	int src_linesize[3] = { data->linesizey,
		data->linesizeu,
		data->linesizev };
	...
	*/
	...
	if (bSel)
	{
		...
		LineArr[0].x = rect.left;
		LineArr[0].y = rect.top;

		LineArr[1].x = rect.right+40;
		LineArr[1].y = rect.top;

		LineArr[2].x = rect.right+40;
		LineArr[2].y = rect.bottom;

		LineArr[3].x = rect.left;
		LineArr[3].y = rect.bottom;

		LineArr[4].x = rect.left;
		LineArr[4].y = rect.top;
